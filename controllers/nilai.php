...
class Nilai {
	...
	public function hitung_bobot($nilai_ipa, $nilai_ips) {
		// contoh masukan dari jurnal
		// $nilai = ['ipa', 'ips', 'mtk'];

		// $this->data['nilai']['ipa'] = [73, 88, 1, 0, 18];
		// $this->data['nilai']['ips'] = [85, 92, 2, 1, 16];
		// $this->data['nilai']['mtk'] = [70, 80, 3, 0, 16];

		$alt = ['ipa', 'ips'];

		$this->data['alt']['ipa'] = $nilai_ipa;
		$this->data['alt']['ips'] = $nilai_ips;

		// echo "data asli <br>";
		for($i = 0; $i < count($alt); $i++) {
			for($j = 0; $j < count($this->data['alt'][$alt[$i]]); $j++) {
				$nilai = $this->data['alt'][$alt[$i]][$j];
				// echo $nilai.' ';
				switch ($j) {
					case '0': 
				...
				$this->matrix_x['alt'][$alt[$i]][$j] = $bobot;
			}
			#echo "<br>";
		}
		...
		// for($i = 0; $i < count($alt); $i++) {
		// 	for($j = 0; $j < count($this->matrix_x['alt'][$alt[$i]]); $j++) {
		// 		echo $this->matrix_x['alt'][$alt[$i]][$j].' ';
		...
		// echo "<br>";
		// echo "nilai max setiap alt<br>";
		$total_kriteria = 5;
		$max = 0;
		for($i = 0; $i < $total_kriteria; $i++) {
			for($j = 0; $j < count($alt); $j++) {
				
				// echo $this->matrix_x['alt'][$alt[$j]][$i].' ';
				$nilai_index = $this->matrix_x['alt'][$alt[$j]][$i];
		...
		for($i = 0; $i < count($alt); $i++) {
			for($j = 0; $j < count($this->matrix_x['alt'][$alt[$i]]); $j++) {
				$this->matrix_r['alt'][$alt[$i]][$j] = $this->matrix_x['alt'][$alt[$i]][$j] / $this->nilai_max[$j];

				// echo $this->matrix_r['alt'][$alt[$i]][$j].' | ';
			}
			// echo "<br>";
		}				
		...
		for($i = 0; $i < count($alt); $i++) {
			for($j = 0; $j < 5; $j++) {
				$nilai_alt += ($this->matrix_r['alt'][$alt[$i]][$j] * $this->w[$j]); 
			}
			$temp[$i] = $nilai_alt;
			// echo "alt $i -> $temp[$i]";
			$nilai_alt = 0;
